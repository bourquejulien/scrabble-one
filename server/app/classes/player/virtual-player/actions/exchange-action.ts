...
export class ExchangeAction implements Action {
    ...
    execute(): Action | null {
        ...
        for (let i = 0; i < exchangeCount; i++) {
            ...
            lettersToExchange.push(this.playerData.rack.splice(indexToReplace, 1)[0]);
        }

        // Add back letters from reserve
        lettersToExchange.forEach(() => this.playerData.rack.push(this.reserve.drawLetter()));
    ...
    private get exchangeSize(): number {
        return this.isRandom
            ? Math.min(this.reserve.length, Math.ceil(Math.random() * this.playerData.rack.length))
            : Math.min(this.playerData.rack.length, this.reserve.length);
    }
}
